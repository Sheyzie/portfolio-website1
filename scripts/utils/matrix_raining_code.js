...
// getting the body DOM dimention OBJ
export const rectForBody = bodyElem.getBoundingClientRect();
...
// set canvas width and height based on body not window
canvas.width = window.innerWidth;
canvas.height =  rectForBody.height// window.innerHeight;

// creating object template for  variables
export class Symbol { // this class will create and draw individual symbol object - create and manage individual symbol
  // initializing constructor
  constructor(x, y, fontSize, canvasHeight) {
    // - x, y coordinates select the position for the symbol on canvas

    this.characters = '丹日亡句ヨ乍呂廾工勹片し冊几回尸甲尺己卞凵レ山メと乙ﾑ乃ᄃり乇ｷムんﾉﾌズﾚﾶ刀のｱゐ尺丂ｲひ√Ｗﾒﾘ乙ค๖¢໓ēfງhiวkl๓ຖ໐p๑rŞtนงຟxฯຊABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
    this.x = x;
    this.y = y;
...
export class Effect { // this class is the main wrapper for the entire rain effect - manage all the symbols at once
  ...
  #initialize() {
    ...
    for(let i = 0; i <= this.columns; i++) {
      ...
      // - i is the x coordinat and y coordinate is 0 which will increase to give falling effect
      // - y
    }
  }
...
export const fps = 10; // frame per sec

// how long before the next frame
export const nextFrame = 1000 / fps; // 15 frames per second
